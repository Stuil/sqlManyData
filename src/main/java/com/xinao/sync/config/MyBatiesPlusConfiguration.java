...
public class MyBatiesPlusConfiguration {
    ...
    public PaginationInterceptor paginationInterceptor() {
        ...
        dynamicTableNameParser.setTableNameHandlerMap(new HashMap<String, ITableNameHandler>(2) {{
            //表集合
            List<String> tables = tablesArray.getTableNames();
            //动态表规则 tableName + myTableName.get() 根据实际情况来，两个组成表名
            tables.forEach(tableTitle -> put(tableTitle,(metaObject, sql, tableName) -> tableName + myTableName.get()));
        }});
        // 动态表名SQL解析器 单个
        /*dynamicTableNameParser.setTableNameHandlerMap(new HashMap<String, ITableNameHandler>(2) {{
            put("gas_user_charge_record_", (metaObject, sql, tableName) -> {
                // metaObject 可以获取传入参数，这里实现你自己的动态规则
                return myTableName.get();
            });
        }});*/
        // set   SQL解析器集合 List<ISqlParser>
    ...
    public static void getTableName(String str){
        myTableName.set(str);
       // myTableName.set("gas_user_charge_record_"+str);
    }
